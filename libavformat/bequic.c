...
static const AVOption bequic_options[] = {
    ...
    { "ietf_draft_version",     "Ietf draft quic version.",    		        OFFSET(ietf_draft_version),     AV_OPT_TYPE_INT,    { .i64 = 0 },   0,   256,       .flags = D|E },
    { "handshark_version",      "QUIC handshark version.",    		        OFFSET(handshark_version),      AV_OPT_TYPE_INT,    { .i64 = 1 },   1,   2,    	    .flags = D|E },
    { "transport_version",      "QUIC transport version.",    		        OFFSET(transport_version),      AV_OPT_TYPE_INT,    { .i64 = 43 },  39,  99, 	    .flags = D|E },
    { "use_ffmpeg_resolve",     "use ffmpeg resolve.",    		            OFFSET(use_ffmpeg_resolve),	    AV_OPT_TYPE_BOOL,   { .i64 = 1 },   0,   1,    	    .flags = D|E },
...
static int quic_open(URLContext *h, const char *uri, int flags) {
    ...
    do {
        ...
        av_log(h, AV_LOG_INFO, "be_quic_open %s, verify:%s, ietf_draft_version:%d, handshark_version:%d, transport_version:%d, use_own_dns:%s, timeout:%d.\n", \
		url, s->verify_certificate?"true":"false", s->ietf_draft_version, s->handshark_version, s->transport_version, s->use_ffmpeg_resolve?"true":"false", s->timeout);
	    ...
	    if (s->use_ffmpeg_resolve) {
            ...
            av_log(h, AV_LOG_INFO, "Android resolve %s to %s:%d\n", hostname, ip, port);
        }
