                    ...
                    AVDictionary *opts, AVDictionary *opts2, int *is_http)
{
    ...
    av_freep(pb);
#if CONFIG_LIBBEQUIC
    https = av_strstart(url, "https", &url_next);
    http = (https == 0) ? av_strstart(url, "http", &url_next) : 0;
    if (https || http) {
        int url_len = strlen(url);
        char *url_quics = av_malloc(url_len + 1);
        memset(url_quics, 0, url_len + 1);
        if (https) {
            av_strlcpy(url_quics, "quics", url_len + 1);
        } else {
            av_strlcpy(url_quics, "quic", url_len + 1);
        }
        av_strlcat(url_quics, url_next, url_len + 1);
        ret = avio_open2(pb, url_quics, AVIO_FLAG_READ, c->interrupt_callback, &tmp);
        av_free(url_quics);
        url_quics = NULL;
        if (ret < 0) {
